...
Fixes vs original
-----------------
- Layer 1 fatal check now uses `not l1["passed"]` instead of
  `l1.get("fatal")` — "fatal" was removed from the Layer 1 result dict
  because it was always equal to `not passed`.
- _save_result_as_json was defined *inside* _build_output as a nested
  function then called as a method (self.x(self, ...)) — moved to a
  proper instance method.
- L1 warnings (e.g. all-null columns, column order) are now surfaced
  in the final output under "warnings".
- _prepare() now applies a post-normalization integer cast alignment:
  if one side has int64 and the other has float64 for the same column
  (common when SQLite returns NULL for some rows), both are cast to
  float64 so hashing is consistent.
"""
...
class ComparatorPipeline:
    ...
    def run(self) -> dict:
        ...
        return self._build_output(
            decision, reasons, talend_df, sql_df, l1, l2, l3, l4
        )
    ...
    def _load_data(self) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        logger.info(f"Loaded '{self.staging_table}': "
                    f"{len(sql_df)} rows x {len(sql_df.columns)} columns")
    ...
    def _prepare(
        ...
        """
        Normalize both DataFrames then infer the schema on the result.

        Post-normalization dtype alignment
        -----------------------------------
        After normalize(), a column that is all-NULL on one side will be
        float64 (NaN) while the other side may be object or int64.  If left
        unaligned, the Layer 2 SHA-256 hash will differ because str(NaN)
        != str(0) etc.  We align numeric dtypes here so both sides produce
        the same string representation for the same logical value.
        """
        ...
        # ── Post-normalization dtype alignment ───────────────────────
        # If both sides now share a column where one is float64 and the
        # other is int64 (or object), cast both to float64 so that NULL
        # rows (NaN) and integer rows compare as equal strings.
        common_cols = set(talend_norm.columns) & set(sql_norm.columns)
        for col in common_cols:
            td = talend_norm[col].dtype
            sd = sql_norm[col].dtype
            if td != sd:
                import pandas.api.types as pat
                if pat.is_numeric_dtype(td) and pat.is_numeric_dtype(sd):
                    # Upcast both to float64 — the common supertype
                    talend_norm[col] = talend_norm[col].astype("float64")
                    sql_norm[col]    = sql_norm[col].astype("float64")
    ...
    def _build_output(
        self, decision, reasons, talend_df, sql_df, l1, l2, l3, l4
    ) -> dict:
        ...
        # Collect all warnings from layers that emit them
        all_warnings = list(l1.get("warnings", []))
        ...
        result = {
            ...
            "structure": {
                ...
                "row_count_match"    : (
                    l2c.get("2.1_row_count", {}).get("passed", False)
                    if l2 else False
                ),
                "column_match"       : (
                    l1["checks"]
                    .get("1.1_column_naming", {})
                    .get("passed", False)
                ),
                "columns_only_in_talend": (
                    l1["checks"]
                    .get("1.1_column_naming", {})
                    .get("missing_in_sql", [])
                ),
            ...
            "row_level": {
                ...
                "matched_rows": (
                    l2c.get("2.2_row_hash", {}).get("matched_hashes", 0)
                    if l2 else 0
                ),
                "diff_rows"   : (
                    l2c.get("2.2_row_hash", {}).get("only_in_talend", 0)
                    if l2 else 0
                ),
                "match_rate"  : match_rate,
        ...
        self._save_result_as_json(result)
        return result

    def _save_result_as_json(
        self, result: dict, output_path: str = "validation_result.json"
    ) -> str:
    ...
    def _print_final(decision, reasons, l1, l2, l3, l4):
        ...
        # Surface L1 warnings even on a passing run
        warnings = l1.get("warnings", [])
        if warnings:
            logger.warning(f"  {len(warnings)} warning(s) from Layer 1:")
            for w in warnings:
                logger.warning(f"    {w}")
